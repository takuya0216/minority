...
/*Cake予約 お渡し日カレンダー設定*/
//選択不可の日付。「yyyy/mm/dd」形式でコンマ区切りで指定。
  var disableDates = [
	  "2023/03/21",
	  "2023/05/03",
	  "2023/05/04",
  ...
  $("#cf7-pickup-date").change(function() {
	...
	//制限
	if(date.getDay() == 1 || fruitcakeOnlyDates.includes(val)){
		//メニュー選択済みの場合処理
		...
		$('#cake-menu-checkbox :checkbox:checked').each(function() {
			...
			if(!enableMenu_Monday.includes(val)){
				...
				Swal.fire({
					icon: 'warning',
					title: 'メニューのご確認',
					html: '申し訳ございません。月曜日、祝日の翌日お渡しの場合は、<br>「季節のフルーツショートケーキ」のみ選択可能となっております。'
				});
			}
		});
		//チェックボックスを選択不可にする
